...
- Navigate from any part to an interactive BOM tree using the plugin navigation entry.
- Limit the rendered depth to keep large BOMs manageable (up to 25 levels).
- Highlight BOM cycles with dotted connections to make modelling issues obvious.
- Optionally include substitute parts directly in the graph labels.
...
- JSON: `/plugin/product_tree/api/tree/<part_id>/?max_depth=10`
  - Append `include_substitutes=1` to list substitute parts inline in the response.
